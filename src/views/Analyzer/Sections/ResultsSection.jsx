...
class ResultsSection extends React.Component {
  ...
  makeSenderQtyData = () => {
    ...
    data.map((record) => {
      record.conversation.map((convo) => {
        ...
        senderData[sender] = senderData[sender] != undefined ? senderData[sender]+1 : 1
      })
    })
  ...
  getMostPopularWords = () => {
    ...
    // 英文停用詞列表
    const stopWords = ['the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'is', 'are', 'was', 'were', 'be', 'been', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'may', 'might', 'must', 'can', 'i', 'you', 'he', 'she', 'it', 'we', 'they', 'me', 'him', 'her', 'us', 'them', 'my', 'your', 'his', 'her', 'its', 'our', 'their', 'this', 'that', 'these', 'those'];
    ...
    wordlist.map((word) => {
        topWrdCount[word] = topWrdCount[word] !== undefined ? topWrdCount[word]+1 : 1
    })
    
    topFiveWords = Object.keys(topWrdCount)
      .sort(function(a,b){return topWrdCount[b]-topWrdCount[a]})
      .slice(0,5)
